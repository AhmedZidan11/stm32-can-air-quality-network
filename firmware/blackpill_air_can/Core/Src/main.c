...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  air_quality_status_t service_status = air_quality_service_process(&g_air_quality, HAL_GetTick());

	  if (service_status != AIR_QUALITY_STATUS_NO_UPDATE)
	  {
...
{
  air_quality_service_on_i2c_rx_complete(&g_air_quality, hi2c);
}
...
{
    MCP2515_SPI_TxRxCplt_Handler(hspi);
}
/* USER CODE END 4 */
